...
public class Deck
{
    ...
    public void Create()
    {
        //first create a list of card data fro the pack.
        List<CardData> cardDateInOrder = new List<CardData>();
        foreach(CardData cardData in GameController.instance.cards)
        {
            for (int i = 0; i < cardData.NumberInDeck; i++)
                cardDateInOrder.Add(cardData);
        }

        //then randomize the order.
        while (cardDateInOrder.Count > 0)
        {
            int randomIndex = Random.Range(0, cardDateInOrder.Count);
            cardDatas.Add(cardDateInOrder[randomIndex]);
            cardDateInOrder.RemoveAt(randomIndex);
        }
    }
